...
const Index = ({ auth, notes, tags, filterTags }: PageProps<{tags: Tag[], notes: Note[], filterTags: number[]}>) => {
    return (
        <>
            <Head title="Notes" />
            <Layout
                    ...
                    <div className="max-w-screen-2xl mx-auto sm:px-4 lg:px-6">
                        <div className="flex flex-col-reverse gap-12 pb-10 lg:flex-row lg:justify-center xl:gap-x-16">
                            <div className="flex-1">
                                ...
                                {/*Search Note*/}
                                <div style={{boxShadow: '0 0 10px 10px #fff'}}
                                     className="m-2 sticky top-[10px] self-start min-w-[300px] xl:min-w-[350px]">
                                    <form method="get">
                                        <label htmlFor="hs-trailing-button-add-on-with-icon-and-button"
                                        ...
                                        <div className="relative flex rounded-lg shadow-sm">
                                            ...
                                            <button type="submit"
                                                    className="py-2 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-e-md border border-transparent bg-indigo-500 text-white hover:bg-indigo-600 disabled:opacity-50 disabled:pointer-events-none">Search
                                            </button>
                                        </div>
                                    </form>
                                </div>
                            ...
                            <aside className="xl:sticky xl:top-[10px] xl:self-start xl:min-w-[350px] hidden lg:block"
                                   style={{flex: '0 1 0'}}>
                                <h3 className="text-2xl dark:text-white mb-4">Tag Filter</h3>
                                <div className="">
                                    <form method="get">
                                        <div className="grid space-y-2">
                                            { tags && tags.map((tag: Tag) => (
                                                <label key={tag.id} htmlFor="tag-filter-{{ $tag->id }}"
                                                    ...
                                                    <span
                                                        className="text-sm text-gray-500 ms-3 dark:text-neutral-400">{tag.name}</span>
                                                </label>
                                            ))}
                                        </div>
                                    </form>
